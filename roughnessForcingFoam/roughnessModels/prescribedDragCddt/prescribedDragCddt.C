...
{
    defineTypeNameAndDebug(prescribedDragCddt, 0);
    addToRunTimeSelectionTable(roughnessModel, prescribedDragCddt, dictionary);


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

prescribedDragCddt::prescribedDragCddt
(
...
:
    ...
    Cddt_("Cddt", dimless, roughnessModelDict.lookup("Cddt")),
...
tmp<fvVectorMatrix> prescribedDragCddt::force(volVectorField& U) const
{
    dimensionedScalar dimFix("dimFix", dimless/dimLength, 1.0);
    
    return
    (
        fvm::SuSp(-dragCoeff_ * Ct_ * dimFix * mag(U), U) - Cddt_*pos(dragCoeff_-SMALL)*fvc::ddt(U) 
        /*
...
void prescribedDragCddt::updateForce(volVectorField& U)
{
    dimensionedScalar dimFix("dimFix", dimless/dimLength, 1.0);
    fi_ = -dragCoeff_ * Ct_ * dimFix * mag(U) * U - Cddt_*pos(dragCoeff_-SMALL)*fvc::ddt(U);
}

...
{
    defineTypeNameAndDebug(prescribedForce, 0);
    addToRunTimeSelectionTable(roughnessModel, prescribedForce, dictionary);
...
tmp<fvVectorMatrix> prescribedForce::force(volVectorField& U) const
{
    dimensionedScalar dimFix("dimFix", dimLength/dimTime/dimTime, 1.0);
    
    return
    (
        - dragCoeff_*dimFix*vector(1.0, 0.0, 0.0) + fvm::Sp(dimensionedScalar("zero", dimless/dimTime, scalar(0.0)),U) 
        /*

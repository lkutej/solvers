...
{
    defineTypeNameAndDebug(geometricHetSqr, 0);
    addToRunTimeSelectionTable(pPFPorosityModel, geometricHetSqr, dictionary);


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

geometricHetSqr::geometricHetSqr
(
...
tmp<fvVectorMatrix> geometricHetSqr::force(volVectorField& U) const
{
    dimensionedScalar Dmin("Dmin", dimensionSet(0,1,0,0,0,0,0), SMALL);

    if(runTime_.outputTime())
    {
        volVectorField fiGeo("fiGeo", -(nu_*2.0*C1_*alpha_/(constant::mathematical::pi*max(sqr((1.0-alpha_)*Dm_),sqr(Dmin))) + 2.0*C2_*alpha_/(constant::mathematical::pi*max((1.0-alpha_)*Dm_,Dmin))*mag(U))*cmptMultiply(vector(Cx_.value(), Cy_.value(), Cz_.value()),U));
        fiGeo.write();
    }
    ...
    (
        ...
        -(nu_*2.0*C1_*alpha_/(constant::mathematical::pi*max(sqr((1.0-alpha_)*Dm_),sqr(Dmin))) + 2.0*C2_*alpha_/(constant::mathematical::pi*max((1.0-alpha_)*Dm_,Dmin))*mag(U))*cmptMultiply(vector(Cx_.value(), Cy_.value(), Cz_.value()),U) + fvm::Sp(dimensionedScalar("zero", dimless/dimTime, scalar(0.0)),U)
    );
}

...
defineTypeNameAndDebug(NakayamakEp, 0);
addToRunTimeSelectionTable(pPFRASModel, NakayamakEp, dictionary);
...
tmp<volSymmTensorField> NakayamakEp::R() const
{
...
tmp<volSymmTensorField> NakayamakEp::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> NakayamakEp::divDevReff(volVectorField& U) const
{
    return
    (
        -1.0/beta_*
...
tmp<fvVectorMatrix> NakayamakEp::divDevRhoReff
(
...
void NakayamakEp::correct()
{
    ...
    (
     ...
     ==
      ...
      - fvm::Sp(CEps2_/k_*epsilon_*beta_, epsilon_)
      + CEps2_*sqr(epsInf)/max(kInf,kMin_)*beta_
    );
